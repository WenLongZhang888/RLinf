...
"""Abstract base class for robot end-effectors (shared across robots)."""
...
class EndEffectorType(str, Enum):
    """Supported end-effector types across real-world robot arms."""

    FRANKA_GRIPPER = "franka_gripper"
    ROBOTIQ_GRIPPER = "robotiq_gripper"
    RUIYAN_HAND = "ruiyan_hand"
    MARVIN_GRIPPER = "marvin_gripper"
    ...
    def is_gripper(self) -> bool:
        return self in (
            ...
            type(self).MARVIN_GRIPPER,
        )

    @property
    def is_hand(self) -> bool:
        return self in (
            type(self).RUIYAN_HAND,
            type(self).REVO2_HAND,
        )
    ...
    def gripper_backend(self) -> str:
        ...
        if self == type(self).ROBOTIQ_GRIPPER:
            return "robotiq"
        if self == type(self).MARVIN_GRIPPER:
            return "marvin"
...
class EndEffector(ABC):
    ...
    Every end-effector must expose its state and action dimensions so the
    owning env can build the correct Gymnasium spaces dynamically.
    """
