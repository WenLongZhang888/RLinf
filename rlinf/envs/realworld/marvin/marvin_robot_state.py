...
class MarvinRobotState:
    # https://docs.ros.org/en/kinetic/api/libmarvin/html/structmarvin_1_1RobotState.html
    ...
    )  # MarvinState.K_F_ext_hat_K[0:3]
    tcp_torque: np.ndarray = field(
        default_factory=lambda: np.zeros(3)
    )  # MarvinState.K_F_ext_hat_K[3:6]
    arm_jacobian: np.ndarray = field(
        default_factory=lambda: np.zeros((6, 7))
    )  # ZeroJacobian.zero_jacobian
    joints_torque: np.ndarray = field(
        default_factory=lambda: np.zeros(7)
    )  # 7 joint torques from SDK get_all_state()
